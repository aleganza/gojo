...
async function fetchEpisodes(tvName: string, type?: string): Promise<Season[]> {
  // Cerca la serie o il film
  ...
  const detailsRes = await fetch(`${TMDB_BASE_URL}/tv/${tv.id}`, { headers: tmdbHeaders });
  if (!detailsRes.ok) throw new Error("TMDB tv details failed");

  const details: TVDetailsResponse = await detailsRes.json();
  const seasons = details.seasons ?? [];

  // Recupera le stagioni con episodi
  const seasonData: Season[] = (
    await Promise.all(
      seasons
        .filter((s) => s.season_number > 0)
        .map(async (season) => {
          const res = await fetch(`${TMDB_BASE_URL}/tv/${tv.id}/season/${season.season_number}`, {
            headers: tmdbHeaders,
          });
          ...
          return {
            ...
            episodes: (data.episodes ?? []).map((ep) => ({
              image: ep.still_path ? `https://image.tmdb.org/t/p/w500${ep.still_path}` : undefined,
              title: ep.name,
              summary: ep.overview,
              airdate: ep.air_date,
              length: ep.runtime,
              episodeNumber: ep.episode_number,
              // seasonNumber: ep.season_number, // opzionale
            })),
          };
        }),
    )
...
// Hook React Query
export function useTMDBEpisodes(tvName: string, type?: string): UseQueryResult<Season[], Error> {
  return useQueryData<Season[], Error>({
