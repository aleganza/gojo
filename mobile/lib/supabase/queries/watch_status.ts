...
export function useUpdateWatchStatus(mediaId: string, userId: string) {
  ...
  return useMutationData<
    Db.WatchStatus,
    {
      status: Db.WatchStatus["status"];
      current_episode?: number | null;
      current_season?: number | null;
    },
    unknown
  >({
    mutationFn: async ({ status, current_episode, current_season }) => {
      ...
      if (!existing) {
        const { data: inserted, error: insertError } = await supabase
          ...
          .insert({
            ...
            current_season: current_season ?? null,
          })
          .select()
          .single();
        ...
        const { data: updatedData, error: updateError } = await supabase
          ...
          .update({
            ...
            current_season: current_season ?? existing.current_season,
...
export function useUpdateCurrentEpisode(mediaId: string, userId: string) {
  const queryKey = ["watchStatus", userId, mediaId];

  return useMutationData<
    Db.WatchStatus,
    { current_episode?: number | null; current_season?: number | null },
    unknown
  >({
    mutationFn: async ({ current_episode, current_season }) => {
      const { data: existing, error } = await supabase
        .from("watch_status")
        .select("*")
        .eq("user_id", userId)
      ...
      if (error && error.code !== "PGRST116") throw error;
      if (!existing) throw new Error("Watch status not found");

      // Costruisci oggetto update dinamicamente
      const updateData: Partial<Db.WatchStatus> = {
        updated_at: new Date().toISOString(),
      };
