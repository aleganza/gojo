...
import {
  ...
  SLIDE_ENTERING,
  SLIDE_EXITING,
} from "@/lib/streaming/player/player-animations";
import { usePlayerStore } from "@/lib/streaming/player/playerStore";
import SubtitlesOverlay from "@/lib/streaming/player/SubtitlesOverlay";
import {
  QualityStream,
  StreamingSource,
  SubtitleTrack,
} from "@/lib/streaming/streamingSource.types";
...
import { hapticVibrate, hapticVibrateSoft } from "@/lib/utils/haptics";
import {
  parseTime,
...
// import { parseInsidePlayerTitle } from '@/lib/player/parse';
...
const PlayerScreen: React.FC = () => {
  ...
  const ctx = usePlayerStore();
  if (ctx.status !== "ready") throw new Error("Player not initialized");

  const { storage } = useStorage();
  const { largerSize, smallerSize } = useScreenSize();
  const { theme } = useTheme();

  const { data: watchStatus } = useWatchStatus(ctx.media.id!, user.id);
  const mutateWatchStatus = useUpdateWatchStatus(ctx.media.id!, user.id);
  const mutateCurrentEpisode = useUpdateCurrentEpisode(ctx.media.id!, user.id);
  ...
  // const autoPiP = useRef(storage.autoPiP as boolean).current;
  ...
  const player = useVideoPlayer(
    ...
    // Asset.fromModule(require("../../assets/mocks/gojo.mp4")).uri,
    ...
    (p) => {
      ...
      if (storage.player.volume && storage.player.playbackRate) {
        p.volume = storage.player.volume;
        p.playbackRate = storage.player.playbackRate;
      } else {
        p.volume = 1;
        p.playbackRate = 1;
      }
  ...
  const updateProgressThreshold = 85;

  const progressPercentage = useMemo(() => {
    return (currentTime / duration) * 100;
  }, [currentTime, duration]);

  const whichTimeWillProgress = useMemo(() => {
    return (duration * updateProgressThreshold) / 100;
  }, [currentTime, duration]);
  ...
  useEventListener(player, "sourceChange", () => {
    ...
    if (!hasOpenedPlayer) changePosition(ctx.startAt);
    setHasOpenedPlayer(true);
  });

  const loadSource = async (): Promise<boolean> => {
    try {
      const streamingSource = ctx.streamingSource;

      if (streamingSource === undefined) return false;
      ...
      setActiveSubtitle(streamingSource?.subtitles?.[0] ?? null);
      setTitle(gt);
      setEpisodeTitle(pt);
      ...
      await player.replaceAsync({
        ...
        metadata: {
          artist: gt,
          artwork: ctx.media.poster_url ?? undefined,
          title: pt,
        },
  ...
  const changeEpisode = async (newNumber: number) => {
    ...
    //   // TODO: here should change episode loading another source
    // } catch (error) {
    //   alert(error instanceof Error ? error.message : String(error));
    //   console.error(error);
    //   closePlayer();
  ...
  useEffect(() => {
    const checkUpdateProgress = () => {
      ...
      if (
        storage.player.syncProgress &&
        progressPercentage >= updateProgressThreshold &&
        !progressUpdated
      ) {
        mutateCurrentEpisode.mutate({
          current_episode: ctx.currentEpisode,
          current_season: ctx.currentSeason,
        });
  ...
  useEffect(() => {
    // TODO:
    // if (!storage.pauseDuringInteractions) return;
  ...
  // TODO:
  const getBestQuality = (qualities: QualityStream[]) => {
    const storedQualityUrl = qualities.find(
      (s) => s.quality === storage.player.quality,
    );
    if (storedQualityUrl !== undefined) return storedQualityUrl;

    const defaultQualityUrl = qualities.find((s) => s.quality === "default");
    if (defaultQualityUrl !== undefined) return defaultQualityUrl;

    const SUBQualityUrl = qualities.find((s) => s.quality === "SUB");
            ...
            <AniView
              style={[
                trackballStyles,
                {
                  ...
                  marginLeft: isDraggingTimeline
                    ? -((trackballSizeMax - trackballSizeMin) / 2)
                    : 0,
                  zIndex: 3,
                  borderRadius: 9999,
                },
      ...
      >
        ...
        >
          <VideoView
            ...
            // nativeControls={storage.useNativeControls as boolean}
          />
          ...
          {storage.player.showDevTools && (
            ...
            >
              ...
              <MonoText>ctx.currentEpisode:{ctx.currentEpisode}</MonoText>
              <MonoText>ctx.currentSeason:{ctx.currentSeason}</MonoText>
              <MonoText>ctx.status: {ctx.status}</MonoText>
              <MonoText>
                progressPercentage: {progressPercentage.toFixed(3)}% (at{" "}
                {secondsToMinutesSeconds(whichTimeWillProgress)})
              </MonoText>
              <MonoText>
            ...
            >
              ...
              {!lockEnabled && (
                ...
                >
                  ...
                  <BottomControls
                    entering={
                      storage.player.timelineAnimation === "fade"
                        ? FADE_ENTERING
                        : SLIDE_ENTERING
                    }
                    exiting={
                  ...
                  >
                    ...
                    >
                      <Txt
                        style={{
                          fontFamily: theme.family.base.medium,
                          color: theme.colors.text,
                          fontSize: 14,
                          marginLeft: 10,
                        }}
...
const MagicSkipButton: React.FC<{
  ...
  return (
    <View style={{ position: "relative" }}>
      ...
      >
        <Txt
          style={{ fontFamily: theme.family.accent.semi_bold, fontSize: 14 }}
        >
...
const MagicSliders: React.FC<{
  ...
  useEffect(() => {
    ...
    const initBrightness = async () => {
      const defaultBrightness = storage.player.autoMaxBrightness
        ? 1
...
const PanSlider: React.FC<{
  ...
  return (
    ...
    >
      <Icon color={theme.colors.text} size={30} />
