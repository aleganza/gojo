...
import { parseTime, timeToSeconds } from "@/lib/utils/time";
...
const PlayerScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    //   if (!storage.episodeProgressThreshold || !duration) return;
    //   const progressPercentage = (currentTime / duration) * 100;
    //   if (progressPercentage >= storage.episodeProgressThreshold && !progressUpdated) {
    //     updateAnimeProgress(media, setUserLists, Number(episodeNumber));
    //     setProgressUpdated(true);
    //   }
    // };
    // if (duration !== 0) updateSecondsProgress(20);
    // if (isLoggedInWithAniList && storage.anilistSyncEnabled && provider === 'anilist')
    //   checkUpdateProgress();
  }, [currentTime]);
